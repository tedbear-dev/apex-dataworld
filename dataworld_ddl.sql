CREATE TABLE  "DW_FUNCTIONS" 
   (	"UUID" VARCHAR2(40) NOT NULL ENABLE, 
	"PACKAGE_NAME" VARCHAR2(1000) NOT NULL ENABLE, 
	"FUNCTION_NAME" VARCHAR2(100) NOT NULL ENABLE, 
	"PURPOSE" VARCHAR2(1000), 
	"PLSQL_CODE" CLOB, 
	 CONSTRAINT "DW_FUNCTIONS_PK" PRIMARY KEY ("UUID")
  USING INDEX  ENABLE, 
	 CONSTRAINT "DW_FUNCTIONS_UK1" UNIQUE ("PACKAGE_NAME", "FUNCTION_NAME")
  USING INDEX  ENABLE
   )
/
CREATE TABLE  "DW_COMPONENTS" 
   (	"ID" NUMBER GENERATED BY DEFAULT ON NULL AS IDENTITY MINVALUE 1 MAXVALUE 9999999999999999999999999999 INCREMENT BY 1 START WITH 1 CACHE 20 NOORDER  NOCYCLE  NOKEEP  NOSCALE  NOT NULL ENABLE, 
	"NAME" VARCHAR2(100) NOT NULL ENABLE, 
	"DESCRIPTION" VARCHAR2(2000), 
	"JAVASCRIPT_CODE" CLOB, 
	"EXTERNAL_URL" VARCHAR2(1000), 
	"AFRAME_COMPONENT" VARCHAR2(100), 
	 CONSTRAINT "DW_COMPONENTS_PK" PRIMARY KEY ("ID")
  USING INDEX  ENABLE, 
	 CONSTRAINT "DW_COMPONENTS_UK1" UNIQUE ("NAME")
  USING INDEX  ENABLE
   )
/
CREATE TABLE  "DW_COMPONENT_FUNCTIONS" 
   (	"COMPONENT_ID" NUMBER NOT NULL ENABLE, 
	"FUNCTION_UUID" VARCHAR2(40) NOT NULL ENABLE, 
	 CONSTRAINT "DW_COMPONENT_FUNCTIONS_PK" PRIMARY KEY ("COMPONENT_ID", "FUNCTION_UUID")
  USING INDEX  ENABLE
   )
/
CREATE TABLE  "DW_SCENES" 
   (	"UUID" VARCHAR2(40) NOT NULL ENABLE, 
	"NAME" VARCHAR2(100) NOT NULL ENABLE, 
	"DESCRIPTION" VARCHAR2(1000), 
	"AFRAME_MARKUP" CLOB, 
	 CONSTRAINT "DW_SCENES_PK" PRIMARY KEY ("UUID")
  USING INDEX  ENABLE
   )
/
CREATE TABLE  "DW_SCENE_COMPONENTS" 
   (	"SCENE_UUID" VARCHAR2(40) NOT NULL ENABLE, 
	"COMPONENT_ID" NUMBER NOT NULL ENABLE, 
	 CONSTRAINT "DW_SCENE_COMPONENTS_PK" PRIMARY KEY ("SCENE_UUID", "COMPONENT_ID")
  USING INDEX  ENABLE
   )
/
CREATE TABLE  "DW_ASSETS" 
   (	"ID" NUMBER GENERATED BY DEFAULT ON NULL AS IDENTITY MINVALUE 1 MAXVALUE 9999999999999999999999999999 INCREMENT BY 1 START WITH 1 CACHE 20 NOORDER  NOCYCLE  NOKEEP  NOSCALE  NOT NULL ENABLE, 
	"FILE_NAME" VARCHAR2(1000) NOT NULL ENABLE, 
	"MIME_TYPE" VARCHAR2(100) NOT NULL ENABLE, 
	"BLOB_CONTENT" BLOB, 
	"FILE_TYPE" VARCHAR2(20) NOT NULL ENABLE, 
	 CONSTRAINT "DW_ASSETS_PK" PRIMARY KEY ("ID")
  USING INDEX  ENABLE
   )
/
CREATE TABLE  "DW_SCENE_ASSETS" 
   (	"SCENE_UUID" VARCHAR2(40) NOT NULL ENABLE, 
	"ASSET_ID" NUMBER NOT NULL ENABLE, 
	"ID" VARCHAR2(1000) NOT NULL ENABLE, 
	"PRELOAD" VARCHAR2(1) NOT NULL ENABLE, 
	"RESPONSE_TYPE" VARCHAR2(30), 
	 CONSTRAINT "DW_SCENE_ASSETS_PK" PRIMARY KEY ("SCENE_UUID", "ID")
  USING INDEX  ENABLE
   )
/
CREATE TABLE  "DW_SCENE_MIXINS" 
   (	"SCENE_UUID" VARCHAR2(40) NOT NULL ENABLE, 
	"ID" VARCHAR2(30) NOT NULL ENABLE, 
	"DEFINITION" VARCHAR2(4000) NOT NULL ENABLE, 
	 CONSTRAINT "DW_SCENE_MIXINS_PK" PRIMARY KEY ("SCENE_UUID", "ID")
  USING INDEX  ENABLE
   )
/
CREATE TABLE  "DW_STATIC_FILES" 
   (	"FILE_NAME" VARCHAR2(1000) NOT NULL ENABLE, 
	"MIME_TYPE" VARCHAR2(100) NOT NULL ENABLE, 
	"FILE_CONTENT" BLOB, 
	 CONSTRAINT "DW_STATIC_FILES_PK" PRIMARY KEY ("FILE_NAME")
  USING INDEX  ENABLE
   )
/
CREATE TABLE  "DW_SETTINGS" 
   (	"NAME" VARCHAR2(30) NOT NULL ENABLE, 
	"VALUE" VARCHAR2(1000), 
	 CONSTRAINT "DW_SETTINGS_PK" PRIMARY KEY ("NAME")
  USING INDEX  ENABLE
   )
/
CREATE TABLE  "DW_FUNCTION_PARAMS" 
   (	"FUNCTION_UUID" VARCHAR2(40) NOT NULL ENABLE, 
	"SEQ" NUMBER NOT NULL ENABLE, 
	"NAME" VARCHAR2(100) NOT NULL ENABLE, 
	"DESCRIPTION" VARCHAR2(1000), 
	 CONSTRAINT "DW_FUNCTION_PARAMS_PK" PRIMARY KEY ("FUNCTION_UUID", "NAME")
  USING INDEX  ENABLE
   )
/
ALTER TABLE  "DW_SCENE_COMPONENTS" ADD CONSTRAINT "DW_SCENE_COMPONENTS_FK1" FOREIGN KEY ("SCENE_UUID")
	  REFERENCES  "DW_SCENES" ("UUID") ON DELETE CASCADE ENABLE
/
ALTER TABLE  "DW_SCENE_COMPONENTS" ADD CONSTRAINT "DW_SCENE_COMPONENTS_FK2" FOREIGN KEY ("COMPONENT_ID")
	  REFERENCES  "DW_COMPONENTS" ("ID") ON DELETE CASCADE ENABLE
/
ALTER TABLE  "DW_SCENE_MIXINS" ADD CONSTRAINT "DW_SCENE_MIXINS_FK" FOREIGN KEY ("SCENE_UUID")
	  REFERENCES  "DW_SCENES" ("UUID") ON DELETE CASCADE ENABLE
/
ALTER TABLE  "DW_SCENE_ASSETS" ADD CONSTRAINT "DW_SCENE_ASSETS_FK1" FOREIGN KEY ("SCENE_UUID")
	  REFERENCES  "DW_SCENES" ("UUID") ON DELETE CASCADE ENABLE
/
ALTER TABLE  "DW_SCENE_ASSETS" ADD CONSTRAINT "DW_SCENE_ASSETS_FK2" FOREIGN KEY ("ASSET_ID")
	  REFERENCES  "DW_ASSETS" ("ID") ON DELETE CASCADE ENABLE
/
ALTER TABLE  "DW_COMPONENT_FUNCTIONS" ADD CONSTRAINT "DW_COMPONENT_FUNCTIONS_FK1" FOREIGN KEY ("COMPONENT_ID")
	  REFERENCES  "DW_COMPONENTS" ("ID") ON DELETE CASCADE ENABLE
/
ALTER TABLE  "DW_COMPONENT_FUNCTIONS" ADD CONSTRAINT "DW_COMPONENT_FUNCTIONS_FK2" FOREIGN KEY ("FUNCTION_UUID")
	  REFERENCES  "DW_FUNCTIONS" ("UUID") ON DELETE CASCADE ENABLE
/
CREATE OR REPLACE EDITIONABLE PACKAGE  "APEX_QUERY" is 
-- Query Parser for Oracle Text 
-- 
-- Quickstart:  Run this file in SQL*Plus  sqlplus user/pw @apex_query.pls 
-- then use functions from this package to translate "end user" queries to Oracle Text 
-- syntax 
--   SELECT * FROM mytable  
--     WHERE CONTAINS (colname, apex_query.simpleSearch('+oracle "release date"')) > 0; 
--   SELECT * FROM mytable  
--     WHERE CONTAINS (colname, apex_query.progRelax('+oracle "release date"')) > 0; 
...
/
CREATE OR REPLACE EDITIONABLE PACKAGE BODY  "APEX_QUERY" is 
...
CREATE OR REPLACE EDITIONABLE PACKAGE  "DW_LOG" as
--
  procedure prn(p_msg in varchar2);
--
end;
/
CREATE OR REPLACE EDITIONABLE PACKAGE BODY  "DW_LOG" as
--
...
CREATE OR REPLACE EDITIONABLE PACKAGE  "DW_SCENE" as
--
  procedure render(p_uuid in varchar2);
--
end;
/
CREATE OR REPLACE EDITIONABLE PACKAGE BODY  "DW_SCENE" as
--
  procedure render(p_uuid in varchar2) is
    l_name varchar2(1000);
    l_description varchar2(4000);
    l_aframe_markup clob;
    l_scene_tag number;
  begin
    begin
      select name,description,aframe_markup
      into l_name,l_description,l_aframe_markup
      from dw_scenes
      where uuid = p_uuid;
    exception
      when no_data_found then
        owa_util.status_line(404);
        htp.p('Scene with the unique identifier '||p_uuid||' could not be found.');
        return;
      when others then
    ...
    htp.title(l_name);
    --
    htp.p('<!-- Standard libraries we require, including A-Frame, jQuery, and Don McCurdy''s useful extras -->');
    dw_utl.static_javascript('jquery-3.3.1.min.js'); -- jQuery, as usual!
    dw_utl.static_javascript('aframe-0.7.0.min.js'); -- Standard A-Frame library
    dw_utl.static_javascript('aframe-extras.min.js'); -- Don McCurdy's extras; https://github.com/donmccurdy/aframe-extras
    dw_utl.static_javascript('dataworld.js'); -- Our own support code
    --
    -- Add the function-proxies for all the functions that the scene's components might use
    htp.p('<script type="text/javascript">');
    for f in (
        select 
          min(f.uuid) as uuid,
          min(f.package_name) as package_name,
          min(f.function_name) as function_name,
         listagg(fp.name,', ') within group (order by fp.seq,fp.name) as function_parameters,
         min(f.purpose) as purpose
       from dw_functions f,dw_function_params fp
       where fp.function_uuid(+) = f.uuid
       and f.uuid in (
         select function_uuid from dw_component_functions where component_id in (
           select component_id from dw_scene_components where scene_uuid = p_uuid
         )
       ) 
       group by f.uuid,f.package_name,f.function_name,f.purpose
       order by f.package_name,f.function_name
    ) loop
      htp.p('DW.createNamespace("'||f.package_name||'").'||f.function_name||' = function('||f.function_parameters||'){');
      htp.p('  var params = {');
      for p in (
          select name 
          from dw_function_params
          where function_uuid = f.uuid
          order by seq,name
      ) loop
        htp.p('    "'||p.name||'": '||p.name||',');
      end loop;
      htp.p('    "scene_uuid":'''||p_uuid||''',');
      htp.p('    "function_uuid":'''||f.uuid||'''');
      htp.p('  };');
      htp.p('  return new Promise(function(resolve,reject){');
      htp.p('    $.ajax('''||dw_utl.get_rest_url||'/function/'||f.uuid||''',{');
      htp.p('      contentType: "application/json",');
      htp.p('      data: JSON.stringify(params),');
      htp.p('      dataType: "json",');
      htp.p('      error: function(xhr,status,error){reject({xhr:xhr,status:status,error:error});},');
      htp.p('      method: "POST",');
      htp.p('      processData: false,');
      htp.p('      success: function(data){resolve(data);}');
      htp.p('    })');
      htp.p('  });');
      htp.p('}');
    end loop;
    htp.p('</script>');
    --
    -- Add the code, or links, for the components this scene wants to use
    htp.p('<!-- Components used by this scene -->');
    for c in (
      select name,description,javascript_code,external_url,aframe_component
      from dw_components c,dw_scene_components sc
      where sc.scene_uuid = p_uuid
      and sc.component_id = c.id
      order by name
    ) loop
      if c.external_url is not null then
        htp.p('<script type="text/javascript" data-component-name="'||c.name||'" src="'||c.external_url||'"></script>');
      else
        htp.p('<script type="text/javascript" data-component-name="'||c.name||'">');
        htp.p('/*');
        htp.p(c.description);
        htp.p('*/');
        htp.p('AFRAME.registerComponent('''||c.aframe_component||''',');
        dw_utl.write_clob(c.javascript_code);
        htp.p(');');
        htp.p('</script>');
      end if;
    end loop;
    ...
    htp.headClose;
    htp.bodyOpen;
    -- Add the scene's description as a comment
    htp.p('<!--');
    htp.p(l_description);
    htp.p('-->');
    --
    -- Does the a-scene tag already exist?
    l_scene_tag := dbms_lob.instr(l_aframe_markup,'<a-scene ');
    --
    if l_scene_tag <= 0 then
      htp.p('<a-scene>');
      htp.p('<a-assets>');
      --
      -- Add any Mixins
      for m in (
        ...
        from dw_scene_mixins
        where scene_uuid = p_uuid
        order by id
      ) loop
        htp.p('<a-mixin id="'||m.id||'" '||m.definition||'></a-mixin>');
      end loop;
      --
      -- Add in the actual Assets that this scene uses 
      for a in (
        select a.id as asset_id,
               ...
               sa.preload,
               sa.response_type
        from dw_assets a,dw_scene_assets sa
        where a.id = sa.asset_id
        and sa.scene_uuid = p_uuid
        order by a.id
      ) loop
        htp.prn('<'||a.file_type||' crossorigin="anonymous" id="'||a.id||'" src="'||dw_utl.get_rest_url||'/asset/'||a.asset_id||'"');
        --
        if a.preload = 'Y' then
          htp.prn(' autoplay preload="auto"');
        end if;
        --
        if a.response_type is not null then
          htp.prn(' response-type="'||a.response_type||'"');
        end if;
        --
        if a.file_type != 'img' then
          htp.p('></'||a.file_type||'>');
        else
          htp.p('>');
        end if;
    ...
    dw_utl.write_clob(l_aframe_markup);
    --
    if l_scene_tag <= 0 then
      htp.p('</a-scene>');
    end if;
...
CREATE OR REPLACE EDITIONABLE PACKAGE  "DW_SETTING" as
--
...
/
CREATE OR REPLACE EDITIONABLE PACKAGE BODY  "DW_SETTING" as
--
...
CREATE OR REPLACE EDITIONABLE PACKAGE  "DW_SQL" as
--
  function validate_plsql(p_plsql in varchar2) return varchar2;
  procedure execute_function(p_function_uuid in varchar2,p_params in out blob);
--
end;
/
CREATE OR REPLACE EDITIONABLE PACKAGE BODY  "DW_SQL" as
--
  function validate_plsql(p_plsql in varchar2) return varchar2 is
    l_csr integer;
    l_err varchar2(4000) := null;
  begin
    l_csr := dbms_sql.open_cursor;
    begin
      dbms_sql.parse(l_csr,'begin '||p_plsql||' end;',dbms_sql.native);
    exception
      when others then l_err := sqlerrm;
    end;
    dbms_sql.close_cursor(l_csr);
    return l_err;
  end;
--
  procedure execute_function(p_function_uuid in varchar2,p_params in out blob) is
    l_csr integer;
    l_err varchar2(4000) := null;
    l_rc integer;
    ...
    l_params apex_json.t_values;
  begin
    apex_json.open_object;
    apex_json.write('uuid',p_function_uuid);
    l_csr := dbms_sql.open_cursor;
    begin
      select package_name,function_name,plsql_code
      into l_package_name,l_function_name,l_plsql_code
      from dw_functions
      where uuid = p_function_uuid;
      --
      apex_json.write('package_name',l_package_name);
      apex_json.write('function_name',l_function_name);
      --
      dbms_sql.parse(l_csr,'begin '||l_plsql_code||' end;',dbms_sql.native);
      --
      if p_params is not null then
        apex_json.parse(l_params,wwv_flow_utilities.blob_to_clob(p_params));
      end if;
      --
      for p in (
        select name
        from dw_function_params
        where function_uuid = p_function_uuid
        order by seq,name
      ) loop
        if p_params is not null then 
          dbms_sql.bind_variable(l_csr,p.name,apex_json.get_varchar2(p.name,p_values => l_params));
        else
          dbms_sql.bind_variable(l_csr,p.name,to_char(null));
        end if;
      end loop;
      --
      l_rc := dbms_sql.execute(l_csr);
    exception
...
CREATE OR REPLACE EDITIONABLE PACKAGE  "DW_UTL" as
--
  ...
  procedure write_clob_array(p_clob in clob);
  function file_size(p_blob in blob) return varchar2;
  function file_icon(p_type in varchar2) return varchar2;
--
  -- From; https://github.com/ATKmain/ATK-APEX/blob/master/PLSQL/string_to_selectable_table_function.sql
  FUNCTION string_to_selectable_table ( p_string      IN VARCHAR2,   p_delimiter   IN VARCHAR2 DEFAULT ':') RETURN dw_str_row_tab PIPELINED;
--
end;
/
CREATE OR REPLACE EDITIONABLE PACKAGE BODY  "DW_UTL" as
--
  /*
...
--
  ...
  begin
    ...
    for f in (
      ...
      delete from dw_static_files
      where file_name = f.filename;
      --
      if sql%rowcount > 0 then
        dw_log.prn('Deleted existing file: '||f.filename);
      end if;
      --
      insert into dw_static_files(file_name,mime_type,file_content)
      values(f.filename,f.mime_type,f.blob_content);
      --
      dw_log.prn('Inserted new file: '||f.filename);
    end loop;
  end;
--
  function get_rest_url return varchar2 is
  begin
    return dw_setting.get_value('PROTOCOL')||'://'||dw_setting.get_value('SERVER_NAME')||':'||dw_setting.get_value('PORT_NUMBER')||dw_setting.get_value('REST_URL');
  end;
--
  procedure static_javascript(p_src in varchar2) is
  begin
    htp.p('<script type="text/javascript" src="'||dw_utl.get_rest_url||'/static/'||p_src||'"></script>');
  end;
--
...
--
  procedure write_clob_array(p_clob in clob) is
    l_offs number := 1;
    l_chunk number := 1024;
    l_buff varchar2(32767);
  begin
    ...
    while l_offs < dbms_lob.getLength(p_clob) loop
      l_buff := dbms_lob.substr(p_clob,l_chunk,l_offs);
      apex_json.write(l_buff);
      --
      l_offs := l_offs + l_chunk;
    end loop;
  end;
--
  function file_size(p_blob in blob) return varchar2 is
  begin
    return to_char(nvl(dbms_lob.getLength(p_blob),0)/1024,'fm999,999,990.00')||' kb';
  end;
--
  function file_icon(p_type in varchar2) return varchar2 is
    l_rc varchar2(100);
  begin
    select decode(p_type,'img','fa-file-image-o','audio','fa-file-audio-o','video','fa-file-video-o','fa-file-o')
    into l_rc 
    from dual;
    --
    return l_rc;
...
--
  FUNCTION string_to_selectable_table ( p_string      IN VARCHAR2,   p_delimiter   IN VARCHAR2 DEFAULT ':')
      RETURN dw_str_row_tab PIPELINED
   AS
      n_row NUMBER := 1 ;
      v_str VARCHAR2(4000) ;
   BEGIN      
